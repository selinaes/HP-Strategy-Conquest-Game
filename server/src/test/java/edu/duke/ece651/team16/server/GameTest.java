...
public class GameTest {
  ...
  //   HashMap<String, ArrayList<HashMap<String, String>>> mockMap = mock(HashMap.class);
  ...
  public void testChooseNumOfPlayersInvalid() throws IOException {
    ...
    Game game = new Game();
    Connection connectionMock = mock(Connection.class);
    // when(connectionMock.recv()).thenReturn("1").thenReturn("5").thenReturn("3");
    ...
    // check the numPlayer and sent message
    // assertEquals(3, game.getNumPlayer());
    verify(connectionMock, times(2)).send("You are the first player! Please set the number of players in this game(Valid player number: 2-4): ");
    verify(connectionMock).send("Invalid number of players");
    //verify(connectionMock).send("You are the first player! Please set the number of players in this game(Valid player number: 2-4): ");
